...
//components
import NoPrev from "../../../images/CE.png";
...
const Card = (props) => {
  ...
  useEffect(() => {
    if (props.details.length > 1) setDropDown(true);
  }, []);
  ...
  const config = (website) => {
    ...
    if (index === -1) return website;
    else return website.substr(0, index);
  };
  ...
  return (
    <>
      <div
        className="card"
        style={
          !props.details[0].host.includes(search) ||
          bw.find((element) => element === props.details[0].host) !== undefined
            ? hideStyle
            : {}
        }
      >
        <div className="primaryCard">
          ...
          <div className="item2">
            <Tooltip title={props.details[0].host} arrow>
              ...
              >
                <img
                  src={
                    "https://logo.clearbit.com/" + config(props.details[0].host)
                  }
                  onError={(e) => {
          ...
          <div className="item3">
            <p>
              {props.started
                ? "End: " + setDate(props.details[0].end)
                : "Start: " + setDate(props.details[0].start)}
            </p>
          </div>
          ...
          <div className="item4">
            <Tooltip title="Duration" placement="left" arrow>
              <p>
                ...
                {days
                  ? days + " days"
                  : new Date(props.details[0].duration * 1000)
                      .toISOString()
                      .substr(11, 5)}
              </p>
            </Tooltip>
          ...
          {!props.started && (
            <div className="item6">
              <Save
                host={props.details[0].host}
                event={props.details[0].event}
              />
            </div>
          )}
          <div className="item8">
            {dropDown && (
              <CollapsableList
                children={props.details.filter((element, key) => key !== 0)}
